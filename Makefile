...
CFLAGS = -Os -std=c99 -Wno-missing-braces
COMPILE = avr-gcc -Wall -DF_CPU=$(F_CPU) $(CFLAGS) -mmcu=$(DEVICE) $(SIMFLAGS)
...
clean:
	rm -f $(OUT).hex $(OUT).lst $(OUT).obj $(OUT).cof $(OUT).list $(OUT).map $(OUT).eep.hex $(OUT).elf dis.s *.o
